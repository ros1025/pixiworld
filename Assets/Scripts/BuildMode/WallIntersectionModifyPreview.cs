...
public class WallIntersectionModifyPreview : IDynamicPreviewSystem
{
    ...
    private List<(Spline, int)> selectedSplines;

    public void StartPreview(Vector3 point, Intersection intersection, List<Wall> walls, PlacementSystem system, InputManager input)
    {
        this.system = system;
        this.input = input;
        width = 0.1f; height = 2f;
        ...
        foreach (Wall wall in walls)
        {
            ...
            for (int i = 0; i < wall.points.Count; i++)
            {
                ...
                if (i == 0 && intersection.junctions.FindIndex(item => item.spline == wall.wall && item.knotIndex == 0) != -1)
                {
                    knot.Position = point;
                    selectedSplines.Add((spline, 0));
                }
                else if (i == wall.points.Count - 1 && intersection.junctions.FindIndex(item => item.spline == wall.wall && item.knotIndex > 0) != -1)
                {
                    knot.Position = point;
                    selectedSplines.Add((spline, 1));
                }
                else knot.Position = wall.points[i];
    ...
    {
        foreach ((Spline, int) spline in selectedSplines)
        {
    ...
    private void UpdatePreviewSpline()
    {
        ...
        foreach ((Spline, int) spline in selectedSplines)
        {
            ...
            if (spline.Item1.Count >= 2)
            {
                for (int currentPointIndex = 1; currentPointIndex < spline.Item1.Count; currentPointIndex++)
                {
                    int vertOffset = currentPointIndex;

                    Vector3 dir = (spline.Item1[vertOffset - 1].Position - spline.Item1[vertOffset].Position);
                    Vector3 side = Vector3.Cross(dir.normalized, Vector3.up);

                    Vector3 p1 = ((Vector3)spline.Item1[vertOffset - 1].Position) - side * 0.1f;
                    Vector3 p2 = p1 + new Vector3(0, 2f, 0);
                    Vector3 p3 = ((Vector3)spline.Item1[vertOffset - 1].Position) + side * 0.1f;
                    Vector3 p4 = p3 + new Vector3(0, 2f, 0);
                    Vector3 p5 = ((Vector3)spline.Item1[vertOffset].Position) - side * 0.1f;
                    Vector3 p6 = p5 + new Vector3(0, 2f, 0);
                    Vector3 p7 = ((Vector3)spline.Item1[vertOffset].Position) + side * 0.1f;
                    Vector3 p8 = p7 + new Vector3(0, 2f, 0);
                ...
                int t25 = (8 * (spline.Item1.Count - 2)) + 4;
                int t26 = (8 * (spline.Item1.Count - 2)) + 6;
                int t27 = (8 * (spline.Item1.Count - 2)) + 7;
                int t28 = (8 * (spline.Item1.Count - 2)) + 7;
                int t29 = (8 * (spline.Item1.Count - 2)) + 5;
                int t30 = (8 * (spline.Item1.Count - 2)) + 4;
