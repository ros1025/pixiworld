...
public class PoolModifyPreview : IDynamicPreviewSystem
{
    ...
    public void StartPreview(Pool selectedPool, PlacementSystem system, InputManager input)
    {
        this.system = system;
        this.input = input;
        ...
        for (int i = 0; i < selectedPool.points.Count; i++)
        {
            AddPoint(i, selectedPool.points[i]);
        }
    }
    ...
    private void UpdatePreviewSpline()
    {
        if (system.dynamicCursor[0].Count >= 2)
        {
            ...
            for (int currentPointIndex = 0; currentPointIndex < system.dynamicCursor[0].Count; currentPointIndex++)
            {
                int vertOffset = currentPointIndex;

                Vector3 dir = (system.dynamicCursor[0][vertOffset].Position - system.dynamicCursor[0][(vertOffset + 1) % system.dynamicCursor[0].Count].Position);
                Vector3 side = Vector3.Cross(dir.normalized, Vector3.up);

                Vector3 p1 = ((Vector3)system.dynamicCursor[0][vertOffset].Position) - side * 0.2f;
                Vector3 p2 = p1 + new Vector3(0, 0.1f, 0);
                Vector3 p3 = ((Vector3)system.dynamicCursor[0][vertOffset].Position) + side * 0.2f;
                Vector3 p4 = p3 + new Vector3(0, 0.1f, 0);
                Vector3 p5 = ((Vector3)system.dynamicCursor[0][(vertOffset + 1) % system.dynamicCursor[0].Count].Position) - side * 0.2f;
                Vector3 p6 = p5 + new Vector3(0, 0.1f, 0);
                Vector3 p7 = ((Vector3)system.dynamicCursor[0][(vertOffset + 1) % system.dynamicCursor[0].Count].Position) + side * 0.2f;
                Vector3 p8 = p7 + new Vector3(0, 0.1f, 0);

                //offset = 8 * calculateRes(currentSplineIndex);
                int offset = 8 * (currentPointIndex);
    ...
    public int GetSplineIndex(Vector3 point)
    {
        ...
        {
            int start = 0; int end = system.dynamicCursor[0].Count;
